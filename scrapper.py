...
def download_images(class_name, image_src_list, quant_images):
    """
    get a list of images and use urlretrieve to
    try download every image in hte list
    """
    # IMPORTANT!: if the class name already exists the function will not download the images
    os.mkdir(class_name)
    ...
    for index, image_src in enumerate(image_src_list):
        try:
            ...
            print('download [OK]:', image_src[:10]+'...')
            images_downloaded += 1
        except:
            print('download [FAILED]:', image_src[:10]+'...')
        if index == quant_images - 1: break
    print(images_downloaded, 'images downloaded from the class', class_name, 'check your class folder.')
                
def get_images_of(class_name, quant_images):
    """
    ...
    for image in image_list:
        try: image_src_list.append(image.attrs['src'])
        except: print('failed to found the source of', image[:10]+'...')
        
    driver.close()
    print(len(image_src_list),'found, starting the download!')
    download_images(class_name, image_src_list, quant_images)

get_images_of('da vinci', 15)
